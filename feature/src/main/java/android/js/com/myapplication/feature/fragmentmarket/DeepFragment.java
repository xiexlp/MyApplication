...
import android.js.com.myapplication.feature.fragmentmarket.sub.CalendarFragment;
import android.js.com.myapplication.feature.fragmentmarket.sub.CourseFragment;
...
public class DeepFragment extends Fragment {
    ...
    private final String[] titles = {"日历", "教程", "资讯"};

    TabLayout deep_tablayout;
    ...
    private void initView(View view) {
        fragmentSize=3;
        ViewPager deep_pager = view.findViewById(R.id.deep_pager);

        deep_tablayout = (TabLayout) view.findViewById(R.id.deep_tablayout);
        for (int i = 0; i < fragmentSize; i++) {
            //fragments.add(new TabFragment());
            deep_tablayout.addTab(deep_tablayout.newTab());
            // topics_tablayout.getTabAt(i).setText(titles[i]);
        }


        deep_pager.setAdapter(new ForumPagerAdapter(getChildFragmentManager()));
        deep_tablayout.setupWithViewPager(deep_pager, false);
        deep_pager.setOffscreenPageLimit(2);

        if(null==deep_tablayout) {
            System.out.println("deep_tablayout为空");
            System.out.println(deep_tablayout);
        }
        for (int j = 0; j < fragmentSize; j++) {
            //fragments.add(new TabFragment());
            // topics_tablayout.addTab(new TabFragment());
            //deep_tablayout.getTabAt(j).setText("deep-"+j);
            deep_tablayout.getTabAt(j).setText(titles[j]);
        }


        deep_tablayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            @Override
