...
public class SocialFragment extends Fragment {
   ...
   // ViewPager forum_topics_pager;
    ...
    private void initView(View view){
        ViewPager forum_topics_pager = view.findViewById(R.id.forum_topics_pager);

        topics_tablayout = (TabLayout) view.findViewById(R.id.topics_tablayout);
        for(int i=0;i<titles.length;i++){
            //fragments.add(new TabFragment());
             topics_tablayout.addTab(topics_tablayout.newTab());
           // topics_tablayout.getTabAt(i).setText(titles[i]);
        }


        forum_topics_pager.setAdapter(new ForumPagerAdapter(getChildFragmentManager()));
        topics_tablayout.setupWithViewPager(forum_topics_pager,false);
        forum_topics_pager.setOffscreenPageLimit(3);
        ...
        topics_tablayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            @Override
        ...
        public Fragment getItem(int position) {
            switch (position) {
                ...
                case 1:
                    ...
                    if (null == repliestFragment) {
                        repliestFragment = new RepliestFragment();
                    }

                    return repliestFragment;
                case 2:

                    if (null == likestFragment) {
                        likestFragment = new LikestFragment();
                    }
                   // entFragment = new EntFragment();
                    return likestFragment;
                default:
                    return newestFragment;
            }
        }
