...
import android.js.com.myapplication.feature.fragmentmarket.sub.FocusFragment;
import android.js.com.myapplication.feature.fragmentmarket.sub.AnnounceFragment;
...
public class NewsFragment extends Fragment {
   ...
   // ViewPager forum_topics_pager;
    ...
    private final String[] titles = { "最新消息", "最喜欢的消息"};
    ...
    private void initView(View view){
        ViewPager news_pager = view.findViewById(R.id.news_pager);

        news_tablayout = (TabLayout) view.findViewById(R.id.news_tablayout);
        for(int i=0;i<titles.length;i++){
            //fragments.add(new TabFragment());
            news_tablayout.addTab(news_tablayout.newTab());
           // topics_tablayout.getTabAt(i).setText(titles[i]);
        }


        news_pager.setAdapter(new ForumPagerAdapter(getChildFragmentManager()));
        news_tablayout.setupWithViewPager(news_pager,false);
        news_pager.setOffscreenPageLimit(3);

        for(int i=0;i<2;i++){
            //fragments.add(new TabFragment());
           // topics_tablayout.addTab(new TabFragment());
           //news_tablayout.getTabAt(i).setText("news"+i);
            news_tablayout.getTabAt(i).setText(titles[i]);
        }
